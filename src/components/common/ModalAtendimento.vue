<template>
  <UiModal :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)" size="medium" :show-close="false">
    <template #title>
      <div class="flex items-center gap-3 w-full">
        <div class="w-10 h-10 rounded-full flex items-center justify-center shrink-0" style="background-color: var(--color-primary)">
    ...
    <div class="flex flex-col -mx-6 -mt-6 mb-6">
      <!-- Banner -->
      <div class="px-6 py-3 flex items-center gap-2 mt-[1px]" style="background-color: var(--color-primary-dark); color: white">
      ...
      <div class="flex px-6 mt-4" style="border-bottom: 1px solid var(--color-border)">
        <button 
          ...
          :style="activeTab === 'dados' ? 'border-color: var(--color-primary); color: var(--color-primary)' : 'border-color: transparent; color: var(--color-text-muted)'"
        >
          Dados
        </button>
        <button 
          ...
          class="px-4 py-2 text-sm font-medium transition-colors border-b-2"
          :style="activeTab === 'historico' ? 'border-color: var(--color-primary); color: var(--color-primary)' : 'border-color: transparent; color: var(--color-text-muted)'"
        >
          Histórico
        </button>
      </div>
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-full status-pill">
              <span class="w-2 h-2 rounded-full" style="background-color: var(--color-status-vencido)"></span>
            ...
            <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-full status-pill opacity-50">
              <span class="w-2 h-2 rounded-full" style="background-color: var(--color-text-muted)"></span>
              <span class="text-xs font-medium" style="color: var(--color-text)">Em acompanhamento</span>
            </div>
            <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-full status-pill opacity-50">
              <span class="w-2 h-2 rounded-full" style="background-color: var(--color-text-muted)"></span>
        ...
        <div>
          ...
          <div class="space-y-3">
            <div class="flex justify-between items-center text-sm pb-2 last:pb-0 data-row">
              ...
              <span class="font-medium" style="color: var(--color-text)">{{ atendimento?.data_oco_formatada || '19/11/2025 - 12:00' }}</span>
            </div>
            <div class="flex justify-between items-center text-sm pb-2 last:pb-0 data-row">
      ...
      <div v-else-if="activeTab === 'historico'" class="h-full flex flex-col flex-1">
        ...
        <div class="space-y-4 overflow-y-auto flex-1 pr-2">
          ...
          <div class="flex gap-4 p-4 rounded-lg history-card">
            <!-- Avatar -->
            <div class="shrink-0">
              <div class="w-10 h-10 rounded-full flex items-center justify-center" style="background-color: var(--color-border)">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex justify-between items-start mb-1">
                <span class="text-sm font-medium" style="color: var(--color-text-muted)">Alexnlv</span>
                <div class="text-right">
                  <span class="block text-xs font-medium" style="color: var(--color-text)">19/11/2025</span>
                  <span class="block text-[10px] uppercase mt-0.5" style="color: var(--color-text-muted)">Ocorrência</span>
                </div>
              </div>
              ...
              <!-- Actions -->
              <div class="flex justify-end gap-2 pt-2 mt-2" style="border-top: 1px solid var(--color-border-subtle)">
                <button class="p-1.5 transition-colors rounded btn-icon-danger" title="Excluir">
                  <Trash2 class="w-3.5 h-3.5" />
                </button>
                <button class="flex items-center gap-1.5 px-2 py-1 text-xs font-medium transition-colors rounded btn-icon-action">
...
import { MessageSquare, X, Calendar, Send, Edit2, MessageSquarePlus, User, Trash2 } from 'lucide-vue-next';
...
const handleConfirmNovoComentario = () => {
  // Aqui entraria a lógica de salvar o comentário
