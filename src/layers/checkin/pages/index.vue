<template>
  ...
  >
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 sm:mb-6">
      <h1 class="text-2xl sm:text-3xl font-bold">Check-in</h1>
      ...
      <div class="relative w-full sm:w-80">
        ...
        <input
          v-model="search"
          type="text"
          placeholder="Buscar fornecedor ou responsável"
    ...
    <UiCard class="p-4 sm:p-6">
      ...
      <div v-else-if="paginatedCheckins.length > 0" class="divide-y divide-[var(--color-border)]">
        <div
          v-for="item in paginatedCheckins"
          :key="item.id"
          class="py-3 sm:py-4 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3"
        >
          <div class="flex-1 min-w-0">
            <p class="font-semibold text-[var(--color-text)] truncate">{{ item.fornecedor }}</p>
          ...
          <div class="flex flex-col items-start sm:items-end gap-1 text-xs text-[var(--color-text-muted)]">
            <UiBadge v-if="item.status" size="small" variant="primary">
              {{ item.status }}
            </UiBadge>
            <span v-if="item.dataCheckin" class="font-medium text-[var(--color-text)]">
              {{ item.dataCheckin }}
            </span>
            <span v-if="item.responsavel">Responsável: {{ item.responsavel }}</span>
          </div>
        </div>
      </div>
      ...
      <UiEmptyState
        ...
        description="Não há registros de check-in para os filtros selecionados."
      >
        <template #icon>
    ...
    <UiPaginacao
      v-if="paginatedCheckins.length > 0 && totalPages > 1"
...
import { useCheckinService } from "../composables/useCheckinService";
...
const filters = computed<CheckinFilters>(() => ({
  search: search.value,
}));
...
const { data: checkinsResponse, status } = fetchCheckins(currentPage, itemsPerPage, filters);
...
const fallbackTotalItems = computed(() => checkinsResponse.value?.data?.totalItems ?? checkins.value.length);
...
const paginatedCheckins = computed(() => {
  const items = checkins.value.filter((item) => {
...
const formatLocation = (item: Checkin) => {
