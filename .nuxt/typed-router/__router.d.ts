...
export type TypedRouteLocationRaw<T extends string = string> =
  ...
  | (Omit<RouteLocationPathRaw, 'path'> & { path?: TypedPathParameter<T> });
...
export type TypedRouteLocationRawFromName<T extends RoutesNamesList, P extends string = string> =
  | (Omit<Exclude<RouteLocationRaw, string>, 'name' | 'params' | 'path'> &
      TypedLocationAsRelativeRaw<T>)
  | (Omit<RouteLocationPathRaw, 'path'> & { path?: TypedPathParameter<P> });
 ...
 * ```
 * */
export type TypedRouteLocation<T extends '/' = '/'> = TypedRouteLocationRawFromName<any, T> &
...
export interface TypedRouter
  extends Omit<
    Router,
    'removeRoute' | 'hasRoute' | 'resolve' | 'push' | 'replace' | 'currentRoute'
  > {
  ...
  resolve<T extends string>(
    ...
    currentLocation?: TypedRouteLocationRaw
  ): TypedRouteLocationFromName<RouteNameFromPath<T>>;
  /**
   ...
   */
  push<T extends RoutesNamesList, P extends string>(
    to: NuxtRoute<T, P>
   ...
   * @param to - Route location to navigate to
   */
  replace<T extends RoutesNamesList, P extends string>(
    to: NuxtRoute<T, P>
  ): Promise<NavigationFailure | void | undefined>;
}
...
export type TypedResolvedMatcherLocation<T extends RoutesNamesList> = {
  name: T;
} & ([RoutesParamsRecordResolved[T]] extends [never]
  ? {}
  : { params: RoutesParamsRecordResolved[T] });
 ...
 * */
export type TypedRouteLocationFromName<T extends RoutesNamesList> = TypedRouteFromName<T> & {
  href: string;
};
